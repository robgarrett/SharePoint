...
# Common Name
$commonName = "CN=rdg-demospo@microsoft.com";

$SecurePassword = ConvertTo-SecureString -String "Pa$$word1" -AsPlainText -Force;

$signedCertParams = @{
	'Subject' = $commonName
	'KeySpec' = 'Signature'
	'FriendlyName' = 'Demo SPO OAUTH Cert'
...
# Clean out old certs.
Get-item Cert:\LocalMachine\my\* | ? { $_.Subject -ieq $commonName } | Remove-Item;

# Create a PFX
$cert = New-SelfSignedCertificateEx @signedCertParams;
$pfxCert = Get-Item Cert:\LocalMachine\my\* | ? { $_.Subject -ieq $commonName };
$certBytes = $pfxCert.Export([Security.Cryptography.X509Certificates.X509ContentType]::Pfx, $SecurePassword);
[System.IO.File]::WriteAllBytes("$script:dp0\bin\debug\privkey.pfx", $certBytes);

# Generate .CER
$certBytes = $pfxCert.Export([Security.Cryptography.X509Certificates.X509ContentType]::Cert);
[System.IO.File]::WriteAllBytes("$script:dp0\bin\debug\pubkey.cer", $certBytes);

# Export ready for Azure.
$bin = $cert.RawData
...
$keyCredentials=ConvertTo-Json @($jsonObj) | Out-File "$script:dp0\bin\debug\keyCredentials.txt"
